...
def dataloader_make(ICGT_tips_train, ICGT_tips_test):
    ...
    shuffler = ShuffleSplit(n_splits=1, test_size=0.1, random_state=42).split(ICGT_tips_train.data, ICGT_tips_train.labels) # shuffles training set to extract a validation set
...
training_file = "Data/fracture_k_sequence_all_edit_train.csv"
...
output = model(ICGT_tips_train.data.float())
truth = ICGT_tips_train.labels
errors = []
outputs = []
truths = []
avg_error = 0
...
print('Output    : ' + str(output[bad_index].item()))
print('Truth     : ' + str(truth[bad_index].item()))
print("xxxxxxxxxxxxxxxxxxxxx")
